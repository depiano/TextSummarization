...
public class Store
{
    ...
    public Store(String name_project,int from_year,int from_month,int at_year,int at_month)
    {
        ...
        this.dir_output_msg=new File("/home/depiano/Scrivania/store_messages/output");
        this.dir_store = new File("/home/depiano/Scrivania/store_messages");
        this.dir_split_msgs=new File("/home/depiano/Scrivania/store_messages/messages");
    ...
    public void formatedOutput(ArrayList<Msg> list_msg) throws IOException
    {
    	
    	if(list_msg==null)throw new SaveMsgNullException("Error: msg object null!");
        if(list_msg.isEmpty())throw new InputEmptySaveMsgException("Error: msg object empty!");
        
		PrintWriter out=new PrintWriter(new FileWriter(this.dir_output_msg+"/formatted.dat",true));
        for(int i=0;i<list_msg.size();i++)
        {
        	Msg line=list_msg.get(i);
        	line.setContent(line.getContent().replaceAll(">", ""));
        	line.setContent(line.getContent().replaceAll("-", ""));
			line.setContent(line.getContent().replaceAll("  ",""));
			line.setContent(line.getContent().replaceAll("\b",""));
			line.setContent(line.getContent().replaceAll("\n",""));
			out.append(line.getContent());
        }
        out.close();
	}
